...
creates a table in a database. If no context is used, the default context of 'main' is used. ```sql
 USE test;
...
removes a table from the database. It does not alter the data_file or the table configuration file. ```sql
 USE test;
drop table test
╔╡dropped                                                                                       ╞╗
║1                                                                                               ║
...
# SHOW TABLES
list all tables in the system. ? maybe by database. But I think its all of them. ```sql  # TODO FIX
 show tables
╔╡database                                     ╞╦╡table                                        ╞╗
║main                                           ║test                                           ║
╚[database]╩[table]╝ ```
...
# SHOW COLUMNS FROM TABLE
list all of the columns of a given table ```sql
 USE test;
...
- if a function is present and no columns are present, a single row will be returned ```sql
 USE test;
SELECT database()
...
If no data is matched in the where, nothing is updated ```sql
 USE test;
...
insert a row of data into the database, columns can be orderd ```sql
 USE test;
...
remove a row from the database based on matching criteria ```sql
 USE test;
