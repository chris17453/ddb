...
class engine:
    ...
    def os_cmd(self,cmd,err_msg):
        ...
        if rc!=0:
            ...
            raise Exception(stringer("# {0}: Exit Code {1} - {2} - {3} ",err_msg,rc,output,err,cmd))
    ...
    def svn_commit_files(self,tables):
        ...
        for table_name in tables:
            ...
            if False==os.path.exists(table.data.repo_dir):
                raise Exception(stringer("SVN Repo Directory not found {0}",table.data.repo_dir))
        ...
        if len(base_dir)!=1:
                raise Exception(stringer("SVN Cannot commit multiple SVN directories at once '{0}'",",".join(base_dir)))
        self.info(stringer("SVN Committing {0}",",".join(files)))
        os.chdir(repo_dir)
        cmd=[   'svn',
                'up']
        cmd.extend(files)
        cmd.extend([
                '--no-auth-cache',
                '--username',stringer('{0}',repo_user),
                '--password',stringer('{0}',repo_password),
                '--non-interactive','--trust-server-cert'
                ])
    ...
    def move_svn_temp_file(self,table,prefix="ddb_"):
        ...
        file_src=self.internal['TEMP_FILES'][data_file]['temp_source']
        file_dst=self.internal['TEMP_FILES'][data_file]['temp_local']
        self.info(stringer("Copying {0} -> {1}",file_dst,data_file))
...
def backup_local_source_files(context):
    ...
    for table_key in context.internal['TEMP_FILES']:
        context.info(stringer("Backing up source table {0}",table_key))
        tmp=context.internal['TEMP_FILES'][table_key]
        table=tmp['table']
        if None != tmp['written']:
            context.duplicate_local_data_file(table)
            context.info(stringer("Temp Source Files backed up {0}->{1}",tmp['origin'],tmp['temp_local']))
def move_svn_tempo_to_repo(context):
    context.info("ENTERING","MOVE SVN FROM TEMP TO REPO")
    for table_key in context.internal['TEMP_FILES']:
        context.info(stringer("Moving Temp to SVN REPO up source table {0}",table_key))
        tmp=context.internal['TEMP_FILES'][table_key]
        table=tmp['table']
        if None != tmp['written']:
            context.move_svn_temp_file(table)
            context.info(stringer("SVN FILE MOVED {0}->{1}",tmp['temp_local'],tmp['origin']))
def delete_local_temp_source_files(context):
    context.info("ENTERING","DELETE LOCAL")
    for table_key in context.internal['TEMP_FILES']:
        context.info(stringer("deleting temp source table {0}",table_key))
        tmp=context.internal['TEMP_FILES'][table_key]
        table=tmp['table']
        if None != tmp['written']:
            context.delete_local_backup_data_file(table)
            context.info(stringer("Temp Source file deleted {0}",tmp['temp_local']))
    ...
    for table_key in context.internal['TEMP_FILES']:
        ...
        if None != tmp['written']:
            if tmp['table'].data.repo_type!='svn':
                ...
                context.info(stringer("Source file restored, temp file deleted {0}->{1}",tmp['temp_local'],tmp['origin']))
            if tmp['table'].data.repo_type=='svn':
...
def commit_local_files(context):
    ...
    for table_key in context.internal['TEMP_FILES']:
        ...
        if tmp['table'].data.repo_type=='svn':
            continue
        if None== tmp['written']:
            context.info(stringer("Commit LOCAL {0}",table_key))
...
def commit_svn_files(context):
    ...
    for repo_dir in svn_repos_by_dir:
        context.info(stringer("Commit SVN {0}",repo_dir))
